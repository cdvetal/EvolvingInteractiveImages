...
vec3 generateRGB(float x, float y){
    float r = (externalVal*((pow(mod(x,externalVal),externalVal)/externalVal)*x));
    float g = bri(y,bri(pow(pow((x+bri(externalVal,externalVal)),max(((externalVal+x)+(x-y)),tan(y))),x),x));
    float b = bri(bri(externalVal,externalVal),noise(x,cos(sin(externalVal))));
    return vec3(r,g,b);
}
