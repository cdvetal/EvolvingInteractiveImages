...
vec3 generateRGB(float x, float y){
    float r = tan(sin((externalVal-noise(externalVal,x))));
    float g = mod(max(externalVal,y),cos(bri(x,mod((y/min(pow(externalVal,pow((max(x,sin(x))-x),noise(x,tan(y)))),externalVal)),x))));
    float b = bri(externalVal,bri(x,externalVal));
    return vec3(r,g,b);
}
