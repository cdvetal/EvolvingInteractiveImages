...
void main() {
    ...
    float g = noise(tan(externalVal),min(externalVal,tan(mod(mod(pow(noise(y,x),max(externalVal,y)),externalVal),(((externalVal-x)-tan((y-sin(externalVal))))-(pow((mod(((x-x)-x),min(min(externalVal,externalVal),externalVal))-sin(externalVal)),y)+pow(externalVal,y)))))));
    float b = (y-tan(y));
