...
vec3 generateRGB(float x, float y){
    float r = audio(externalVal,audio(externalVal,tan(x)));
    float g = ((audio(tan((min(audio(x,(externalVal/x)),x)+cos((tan(x)/sin((sin(x)*((min(x,(externalVal-x))*x)/y))))))),audio(externalVal,x))/pow(externalVal,tan(externalVal)))/((y-x)+x));
    float b = tan(pow(externalVal,x));
    return vec3(r,g,b);
}
