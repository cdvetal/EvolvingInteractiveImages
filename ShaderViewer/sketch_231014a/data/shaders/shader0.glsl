...
vec3 generateRGB(float x, float y){
    float r = noise(externalVal,y);
    float g = mod(externalVal,(y+mod(x,x)));
    float b = (max(x,(sin(mod((externalVal*max(max(tan(max(cos(x),x)),(sin(y)-externalVal)),(y*y))),y))/((audio(pow((x*x),x),noise(externalVal,x))+externalVal)+max((x/cos(y)),(x-x)))))/y);
    return vec3(r,g,b);
}
