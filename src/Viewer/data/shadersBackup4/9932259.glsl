...
vec3 generateRGB(float x, float y){
    float r = pow(sin((externalVal-mod(externalVal,sin(audio(min(y,y),x))))),(audio(y,noise(y,externalVal))/cos(externalVal)));
    float g = tan(externalVal);
    float b = mod(((x-tan(cos((noise(x,externalVal)-x))))-(x/min(y,min(min(y,(y*externalVal)),x)))),(max(y,(audio((y-y),x)*(x/audio(min((externalVal+(min(cos(x),min(pow(mod(max(externalVal,externalVal),y),noise(audio(min(max(y,mod((cos(externalVal)-noise(cos(max(noise(x,x),x)),min(y,y))),x)),audio((noise(audio(x,noise(sin(externalVal),x)),mod(x,pow(max(pow(x,((y+noise(x,(noise(y,max(externalVal,cos(tan(externalVal))))+externalVal)))/(externalVal+x))),max((y-y),tan(noise(y,y)))),externalVal)))-externalVal),cos((y*pow(externalVal,externalVal))))),sin(x)),sin(externalVal))),externalVal))+((x-(x/externalVal))*tan(max(max(externalVal,audio(y,x)),(cos(y)/x)))))),y),(pow(x,x)/y)))))+tan(externalVal)));
    return vec3(r,g,b);
}
