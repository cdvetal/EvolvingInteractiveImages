...
vec3 generateRGB(float x, float y){
    float r = ((y/externalVal)*((y+((y*(x-externalVal))*y))*(externalVal/(y-(bri(y,x)/((x/y)-externalVal))))));
    float g = bri(y,((y-(y/((y-x)/externalVal)))*((y/externalVal)+(y/(externalVal*bri(externalVal,externalVal))))));
    float b = ((bri((x*y),(externalVal+x))-((externalVal+y)-(externalVal/y)))/bri((externalVal/x),bri(x,externalVal)));
    return vec3(r,g,b);
}
