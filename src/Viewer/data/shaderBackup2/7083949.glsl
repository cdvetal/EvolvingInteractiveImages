...
vec3 generateRGB(float x, float y){
    float r = audio(externalVal,audio(externalVal,(externalVal*x)));
    float g = ((audio(cos((min((cos(externalVal)*((x+x)-(y+y))),x)+sin(sin(tan(x))))),pow(externalVal,x))/pow(externalVal,tan(externalVal)))/((y-x)+x));
    float b = max(((y/y)*externalVal),externalVal);
    return vec3(r,g,b);
}
