...
vec3 generateRGB(float x, float y){
    float r = audio(audio(bri(externalVal,audio(bri(y,x),y)),externalVal),y);
    float g = (y/audio((x/externalVal),audio((x-x),audio(audio(audio(externalVal,(y/(audio(bri(x,bri((externalVal*(audio(externalVal,x)/y)),x)),x)-y))),((x*y)+x)),audio(y,x)))));
    float b = (externalVal/(y+(externalVal+x)));
    return vec3(r,g,b);
}
