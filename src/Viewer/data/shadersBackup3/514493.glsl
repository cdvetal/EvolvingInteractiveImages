...
vec3 generateRGB(float x, float y){
    float r = (y-x);
    float g = audio(cos(x),sin(cos(externalVal)));
    float b = pow(audio(audio((externalVal*(audio(pow(y,externalVal),y)*y)),pow(x,cos(audio(x,externalVal)))),audio(y,sin(bri(x,externalVal)))),(tan((y-x))+x));
    return vec3(r,g,b);
}
