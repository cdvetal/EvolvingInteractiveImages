...
vec3 generateRGB(float x, float y){
    float r = audio(x,y);
    float g = bri(bri(audio(audio(audio(externalVal,x),x),audio(x,bri(y,bri(audio(externalVal,x),x)))),audio(y,externalVal)),x);
    float b = bri(bri(audio(audio(x,externalVal),externalVal),audio(bri(y,bri(externalVal,x)),y)),bri(externalVal,audio(externalVal,y)));
    return vec3(r,g,b);
}
