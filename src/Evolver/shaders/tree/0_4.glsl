...
vec3 generateRGB(float x, float y){
    float r = min(noise(x,tan(x)),noise(externalVal,min(externalVal,sin(tan(x)))));
    float g = min(noise(0.23747945,tan(externalVal)),noise(x,min(x,sin(tan(y)))));
    float b = min(noise(0.32217073,tan(y)),noise(externalVal,min(x,sin(tan(y)))));
    return vec3(r,g,b);
}
