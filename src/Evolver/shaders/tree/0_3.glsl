...
vec3 generateRGB(float x, float y){
    float r = sin(min(noise(x,tan(x)),noise(externalVal,min(externalVal,sin(tan(x))))));
    float g = sin(min(noise(0.23747945,tan(externalVal)),noise(x,min(x,sin(tan(y))))));
    float b = sin(min(noise(0.32217073,tan(y)),noise(externalVal,min(x,sin(tan(y))))));
    return vec3(r,g,b);
}
