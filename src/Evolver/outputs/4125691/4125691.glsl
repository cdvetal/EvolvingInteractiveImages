...
vec3 generateRGB(float x, float y){
    float r = min(pow(pow(tan((audio(0.5339037,x)/x)),y),y),x);
    float g = min(pow(pow(tan((audio(y,externalVal)/y)),0.5747453),y),externalVal);
    float b = min(pow(pow(tan((audio(y,externalVal)/x)),x),x),x);
    return vec3(r,g,b);
}
