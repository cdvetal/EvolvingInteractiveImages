...
vec3 generateRGB(float x, float y){
    float r = div(aud(0.6529794,x),sub(max(0.23752761,sin(sub(y,sub(y,div(mul(bri(y,y),add(0.9814539,0.85289264)),0.7188339))))),max(x,0.3797922)));
    float g = div(aud(x,x),sub(max(0.10256767,sin(sub(x,sub(0.97586536,div(mul(bri(0.6600003,0.12865853),add(y,x)),x))))),max(0.41249704,x)));
    float b = div(aud(0.6767411,0.38077354),sub(max(x,sin(sub(0.39788294,sub(x,div(mul(bri(x,0.33365822),add(x,0.02228403)),x))))),max(x,x)));
    return vec3(r,g,b);
}
