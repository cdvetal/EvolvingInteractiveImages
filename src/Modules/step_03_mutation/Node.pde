...
class Node {
  ...
  Node(float _mathType, Node[] _childrenNodes, float [] _scalar, int _nodeIndex, int _depth) {
    mathType = _mathType;
  ...
  Node getCopy() {
    ...
    return new Node(mathType, childrenNodesCopy, scalar.clone(), nodeIndex, depth);
  }
}
