...
int shaderChangeLineStart = 82; //3 lines need changing (r,g,b), first line is this
...
void setup() {
  ...
  println(splitStringAtIndex("01234567", 4));
...
void drawPopulation(float _external, float[] _audioSpectrum) {
  ...
  for (int i = 0; i < population.getSize(); i++) {
    ...
    image(population.getIndividual(i).getPhenotype(resolution, _external, _audioSpectrum), x, y, d, d);
