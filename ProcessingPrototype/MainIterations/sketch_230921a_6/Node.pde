class Node {
  ...
  void identify(int _parentIndex, int _depth, Individual _indiv) { //parentID refers to individual nodes[ID]
    breadth = _indiv.getBreadth(_parentIndex, depth);
    depth = _depth;
    if (aNode != null) aNode.identify(_parentIndex, _depth + 1, _indiv);
    if (bNode != null) bNode.identify(_parentIndex, _depth + 1, _indiv);
  }
  ...
  Node getNode(int _depth, int _breadth){
    if(depth == _depth && breadth == _breadth) return this;
    
    if(aNode != null){
     Node potencialNode = aNode.getNode(_depth, _breadth);
     if(potencialNode != null) return potencialNode;
    }
    
    if(bNode != null){
     Node potencialNode = bNode.getNode(_depth, _breadth);
     if(potencialNode != null) return potencialNode;
    }
