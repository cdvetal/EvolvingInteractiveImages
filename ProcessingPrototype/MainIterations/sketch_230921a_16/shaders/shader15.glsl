...
vec3 generateRGB(float x, float y){
    float r = max(max((externalVal+y),x),x);
    float g = mod(mod(externalVal,x),((externalVal/externalVal)/noise(x,audio(audio(audio(x,min(y,y)),x),x))));
    float b = cos((noise(y,(x+externalVal))/x));
    return vec3(r,g,b);
}
