...
vec3 generateRGB(float x, float y){
    ...
    float g = pow(x,(pow(pow(externalVal,x),((externalVal*externalVal)/min(x,pow(audio(y,noise(y,x)),x))))+externalVal));
    float b = mod(((pow(y,(x*externalVal))/x)+externalVal),externalVal);
    return vec3(r,g,b);
}
