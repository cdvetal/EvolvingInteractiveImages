...
vec3 generateRGB(float x, float y){
    float r = sin(sin(externalVal));
    float g = mod(y,noise(pow(y,pow(x,(y+audio(tan(x),y)))),((audio(pow(externalVal,pow(y,audio(externalVal,(tan(externalVal)/externalVal)))),externalVal)+x)/y)));
    float b = cos((x*externalVal));
    return vec3(r,g,b);
}
