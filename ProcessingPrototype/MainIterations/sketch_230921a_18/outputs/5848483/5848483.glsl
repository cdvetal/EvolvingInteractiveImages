...
vec3 generateRGB(float x, float y){
    float r = (tan(mod(y,(y/x)))/x);
    float g = (tan(noise(audio(y,externalVal),min(x,mod(externalVal,min(min((min(max(y,externalVal),noise(externalVal,x))-bri((x/x),audio(x,mod(pow(externalVal,(y/x)),tan(y))))),(externalVal/(min(externalVal,audio((y+y),x))*mod(pow(max(bri((min(mod(y,x),externalVal)-cos(x)),x),x),x),pow(noise(tan(externalVal),x),(pow(y,(y/x))*(pow((bri(max(x,externalVal),externalVal)/externalVal),x)/bri((externalVal/pow(sin(externalVal),externalVal)),mod((y+y),(x+min(y,externalVal))))))))))),y)))))/x);
    float b = audio(audio(x,max(x,y)),y);
    return vec3(r,g,b);
}
