...
vec3 generateRGB(float x, float y){
    float r = audio(tan((tan((audio(cos(externalVal),x)*externalVal))+(x+externalVal))),cos(cos(tan((externalVal*externalVal)))));
    float g = (sin(externalVal)+((externalVal-audio(((y+sin(externalVal))*(y/y)),externalVal))*(externalVal/audio((audio(y,tan(x))/x),(externalVal*(y/cos(x)))))));
    float b = (externalVal-externalVal);
    return vec3(r,g,b);
}
