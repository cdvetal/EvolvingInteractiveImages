...
vec3 generateRGB(float x, float y){
    float r = (tan(sin(x))+((cos(externalVal)/(tan(cos(externalVal))-x))/sin((x+sin((externalVal/audio(y,(externalVal/x))))))));
    float g = ((audio(((x*externalVal)*externalVal),((x+(sin(audio(audio(y,externalVal),externalVal))+(audio(externalVal,(y/externalVal))-y)))+((y+y)/x)))+y)-externalVal);
    float b = audio((sin((tan(x)/y))*y),cos(externalVal));
    return vec3(r,g,b);
}
