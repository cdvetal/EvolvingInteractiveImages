...
vec3 generateRGB(float x, float y){
    float r = mod(x,mod(x,externalVal));
    float g = cos((externalVal+(y+(mod(x,sin(y))+(sin(externalVal)*(mod(y,tan(((externalVal-(externalVal+externalVal))/y)))/(externalVal*y)))))));
    float b = (audio(((y+(audio(mod((externalVal/externalVal),x),(y*sin((externalVal/y))))-x))/y),audio(y,((externalVal*x)+y)))/(audio(externalVal,externalVal)*cos(x)));
    return vec3(r,g,b);
}
