...
vec3 generateRGB(float x, float y){
    float r = mod(mod((sin(externalVal)*y),externalVal),mod(y,y));
    float g = noise((cos(externalVal)*y),pow(audio(y,x),(y-noise(mod((externalVal/sin(noise(x,externalVal))),pow(max(sin((y*y)),noise(noise(x,mod(audio(((y*(externalVal*y))/(pow(mod(sin(y),externalVal),x)+x)),x),externalVal)),y)),mod(y,((y-(externalVal*y))*(audio(noise((externalVal/y),externalVal),x)+x))))),x))));
    float b = sin(x);
    return vec3(r,g,b);
}
