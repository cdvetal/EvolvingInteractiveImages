...
vec3 generateRGB(float x, float y){
    float r = bri(y,audio(externalVal,y));
    float g = mod(x,audio(max((externalVal+externalVal),noise(x,x)),y));
    float b = pow(externalVal,mod(max(min(sin(externalVal),externalVal),x),x));
    return vec3(r,g,b);
}
