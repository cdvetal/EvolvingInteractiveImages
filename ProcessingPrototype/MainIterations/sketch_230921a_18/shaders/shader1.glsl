...
vec3 generateRGB(float x, float y){
    float r = (tan(noise(externalVal,externalVal))+x);
    float g = pow(x,externalVal);
    float b = max(externalVal,(externalVal*audio((bri(externalVal,bri(min(y,y),externalVal))/(y/x)),y)));
    return vec3(r,g,b);
}
