...
vec3 generateRGB(float x, float y){
    float r = mod(x,x);
    float g = (externalVal+externalVal);
    float b = pow((mod(min(tan((x+externalVal)),x),cos(mod(audio((externalVal/x),cos(y)),externalVal)))+((y*(tan(x)+y))+(externalVal*max(cos(x),externalVal)))),x);
    return vec3(r,g,b);
}
