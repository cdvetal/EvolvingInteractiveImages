...
vec3 generateRGB(float x, float y){
    float r = cos(bri(max(((y/pow(max(tan(cos(x)),(externalVal-max(x,y))),y))-min(x,x)),cos(min(min(x,y),x))),sin(x)));
    float g = bri(sin(max(cos(x),y)),x);
    float b = sin(pow(x,max(audio(x,x),pow(x,y))));
    return vec3(r,g,b);
}
