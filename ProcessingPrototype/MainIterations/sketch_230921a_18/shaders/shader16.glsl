...
vec3 generateRGB(float x, float y){
    float r = min(noise(sin(y),x),cos(x));
    float g = min((externalVal-externalVal),(max(bri(y,audio(mod(y,tan(y)),externalVal)),sin(x))*mod(audio(x,y),bri(audio(y,y),cos(externalVal)))));
