...
void setup() {
  ...
  grid = calculateGrid(populationSize, 0, 0, width, height, 30, 10, 30, true);
}
...
void drawPopulation() {
  ...
  for (int i = 0; i < population.getSize(); i++) {
    ...
    if (mouseX > x && mouseX < x + d && mouseY > y && mouseY < y + d) {
      ...
      strokeWeight(6);
      rect(x, y, d, d);
    } else if (population.getIndividual(i).getFitness() > 0) {
      stroke(1);
...
void mousePressed(){
  ...
  hoveredIndividual.giveFitness();
  println("gave fitness");
}
